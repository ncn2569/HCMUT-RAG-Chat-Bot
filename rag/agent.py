...
def run_agents(query, history):
    ...
    if plan.get("primary_action"):
        ...
        print(
            f" [DEBUG - TOOL] LLM quyết định dùng tool: '{tool_name}' với câu hỏi: '{action_query}'"
        )
    ...
    answer = _run_agents(query, context_str)
    if "tôi không tìm thấy thông tin này trong cơ sở dữ liệu." in answer.lower():
        print(
            " [DEBUG - REFLECTION] LLM báo không đủ thông tin từ nguồn chính, kích hoạt Fallback Web Search..."
        )
        ...
        if web_contexts := search_web(web_query):
            ...
            print(
                " [DEBUG - RE-GENERATE] Bắt đầu tổng hợp lại câu trả lời với bối cảnh mới..."
            )
            answer = _run_agents(query, context_str)
    return answer


# TODO Rename this here and in `run_agents`
def _run_agents(query, context_str):
